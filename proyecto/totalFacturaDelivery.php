...
include("conexion.php");	
	...
	$totalFacturaDomicilio = $_POST['totalFacturaDomicilio']; // total con delivery
...
if(mysql_num_rows($resultado)>0) {	
	$eliminaFacturaActiva = "DELETE FROM facturacompra WHERE id_user=(SELECT id FROM usuarios WHERE usuario='$usuario' and activo=1) and activo=1";
		...
		$facturaRetiro = "INSERT INTO facturacompra(id_user,activo,total,total_productos) 
						VALUES ((SELECT id FROM usuarios WHERE usuario='$usuario' and activo=1),1,'$totalFacturaDomicilio','$totalFacturaRetiro')";
